...
class Var(object):
    ...
    if 'DYNO' in environ:
        ON_HEROKU = True
        APP_NAME = str(getenv('APP_NAME', "logo-maker-bot"))
    
    else:
        ON_HEROKU = False
    FQDN = str(getenv('FQDN', aaott.dev)) if not ON_HEROKU or getenv('FQDN') else APP_NAME+'.herokuapp.com'
    ...
    UPDATES_CHANNEL = str(getenv('UPDATES_CHANNEL', "anumitultrabots"))
